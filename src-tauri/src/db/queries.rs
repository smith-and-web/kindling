...
pub fn get_project(conn: &Connection, id: &Uuid) -> Result<Option<Project>> {
    ...
    if let Some(row) = rows.next()? {
        Ok(Some(Project {
            ...
            name: row.get(1)?,
            source_type: SourceType::from_str(&row.get::<_, String>(2)?).unwrap_or(SourceType::Markdown),
...
pub fn get_chapters(conn: &Connection, project_id: &Uuid) -> Result<Vec<Chapter>> {
    ...
    let chapters = stmt.query_map(params![project_id.to_string()], |row| {
        Ok(Chapter {
            id: Uuid::parse_str(&row.get::<_, String>(0)?).unwrap(),
...
pub fn get_scenes(conn: &Connection, chapter_id: &Uuid) -> Result<Vec<Scene>> {
    ...
    let scenes = stmt.query_map(params![chapter_id.to_string()], |row| {
        Ok(Scene {
            id: Uuid::parse_str(&row.get::<_, String>(0)?).unwrap(),
...
pub fn get_beats(conn: &Connection, scene_id: &Uuid) -> Result<Vec<Beat>> {
    ...
    let beats = stmt.query_map(params![scene_id.to_string()], |row| {
        Ok(Beat {
            id: Uuid::parse_str(&row.get::<_, String>(0)?).unwrap(),
...
pub fn get_characters(conn: &Connection, project_id: &Uuid) -> Result<Vec<Character>> {
    ...
    let mut characters: Vec<Character> = stmt.query_map(params![project_id.to_string()], |row| {
        Ok(Character {
            id: Uuid::parse_str(&row.get::<_, String>(0)?).unwrap(),
            project_id: Uuid::parse_str(&row.get::<_, String>(1)?).unwrap(),
            name: row.get(2)?,
    ...
    for character in &mut characters {
        let mut attr_stmt = conn.prepare(
            "SELECT key, value FROM character_attributes WHERE character_id = ?1"
        )?;

        let attrs: Vec<(String, String)> = attr_stmt.query_map(
            params![character.id.to_string()],
...
pub fn get_locations(conn: &Connection, project_id: &Uuid) -> Result<Vec<Location>> {
    ...
    let mut locations: Vec<Location> = stmt.query_map(params![project_id.to_string()], |row| {
        Ok(Location {
            id: Uuid::parse_str(&row.get::<_, String>(0)?).unwrap(),
            project_id: Uuid::parse_str(&row.get::<_, String>(1)?).unwrap(),
            name: row.get(2)?,
    ...
    for location in &mut locations {
        let mut attr_stmt = conn.prepare(
            "SELECT key, value FROM location_attributes WHERE location_id = ?1"
        )?;

        let attrs: Vec<(String, String)> = attr_stmt.query_map(
            params![location.id.to_string()],
            |row| Ok((row.get(0)?, row.get::<_, Option<String>>(1)?.unwrap_or_default()))
        )?
        .filter_map(|r| r.ok())
        .collect();
...
pub fn add_scene_character_ref(conn: &Connection, scene_id: &Uuid, character_id: &Uuid) -> Result<()> {
    conn.execute(
...
pub fn add_scene_location_ref(conn: &Connection, scene_id: &Uuid, location_id: &Uuid) -> Result<()> {
    conn.execute(
...
pub fn get_scene_characters(conn: &Connection, scene_id: &Uuid) -> Result<Vec<Uuid>> {
    let mut stmt = conn.prepare(
        "SELECT character_id FROM scene_character_refs WHERE scene_id = ?1"
    )?;

    let ids = stmt.query_map(params![scene_id.to_string()], |row| {
...
pub fn get_scene_locations(conn: &Connection, scene_id: &Uuid) -> Result<Vec<Uuid>> {
    let mut stmt = conn.prepare(
        "SELECT location_id FROM scene_location_refs WHERE scene_id = ?1"
    )?;
