...
pub fn get_dismissed_suggestions(conn: &Connection, scene_id: &Uuid) -> Result<Vec<(Uuid, Uuid)>> {
    let mut stmt = conn
        .prepare("SELECT scene_id, reference_id FROM dismissed_suggestions WHERE scene_id = ?1")?;
...
pub fn dismiss_suggestion(conn: &Connection, scene_id: &Uuid, reference_id: &Uuid) -> Result<()> {
    conn.execute(
