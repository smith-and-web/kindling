# Kindling

**Kindling** is a story outlining and planning tool for writers. Import your existing outlines from Plottr, Markdown, yWriter, or Longform/Obsidian, then export a manuscript, reading copy, or project for another writing app.

> **Note:** Kindling is currently in active development. Features and documentation may change.

## Documentation

### Getting Started

- [Installation](Installation) - Download and install Kindling
- [Importing Projects](Importing-Projects) - How to import outlines and source projects
- [Exporting Projects](Exporting-Projects) - Choose an output format and export your writing
- [Export Workspace](Export-Workspace) - Customize exports and save profiles for each writing routine
- [Sync and Reimport](Sync-and-Reimport) - Keep source-backed projects up to date

### Writing & Planning

- [Scene Workflow](Scene-Workflow) - Beats, prose, synopsis, and scene metadata
- [References](References) - Track reference types and link them to scenes
- [Settings](Settings) - App and project settings that affect exports

### Reference

- [Supported Formats](Importing-Projects#supported-formats) - Overview of all import formats
- [Troubleshooting](Importing-Projects#troubleshooting) - Common issues and solutions

## Quick Links

- [GitHub Repository](https://github.com/smith-and-web/kindling)
- [Report an Issue](https://github.com/smith-and-web/kindling/issues)

---

_This wiki is automatically synced from the [docs/](https://github.com/smith-and-web/kindling/tree/main/docs) folder in the main repository._
