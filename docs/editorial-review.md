...
Enter the name you want shown with your feedback. This is an attribution label,
not an account or verified identity.
...
annotation controls to move through the work. **Show markup** controls whether
changes are marked while reading. The underlying suggested text remains editable.
...
**Export feedback** creates a `.kindling-feedback` file. Return it to the writer.
...
Inspect suggestions individually or use the explicitly scoped **Accept visible
suggestions** and **Reject visible suggestions** actions. Formatting is preserved
and shown in the comparison. Accepting changes saves the previous scene prose in
**Revisions → Draft history**. Locked scenes must be unlocked first.
...
Writers can use **Send replies to your editor → Export replies and decisions**
...
them. The sample's local **Revisions** panel remains read-only for locked scenes.
...
Create a new **Sample Project** to explore the revision examples in *The Letter*:
